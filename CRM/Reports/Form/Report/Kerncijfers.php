	...
	private function _getCount($type, $from = null, $to = null) {
		...
		switch ($type) {
			...
			case 'lid_sp_end':
				if (!$from || !$to)
					return false;
				$query = 'SELECT COUNT(*) FROM civicrm_membership WHERE membership_type_id IN (' . $this->_membershipType('Lid SP') . ',' . $this->_membershipType('Lid SP en ROOD') . ') AND status_id IN (' . $this->_membershipStatus('Cancelled') . ',' . $this->_membershipStatus('Expired') . ',' . $this->_membershipStatus('Deceased') . ',' . $this->_membershipStatus('Grace') . ')  AND end_date >= "' . $from->format('Y-m-d') . '" AND end_date <= "' . $to->format('Y-m-d') . '"';
				//				echo $query;
				return CRM_Core_DAO::singleValueQuery($query);
				break;
			...
			case 'lid_rood_end':
				if (!$from || !$to)
					return false;
				$query = 'SELECT COUNT(*) FROM civicrm_membership WHERE membership_type_id IN (' . $this->_membershipType('Lid ROOD') . ',' . $this->_membershipType('Lid SP en ROOD') . ') AND status_id IN (' . $this->_membershipStatus('Cancelled') . ',' . $this->_membershipStatus('Expired') . ',' . $this->_membershipStatus('Deceased')  . ',' . $this->_membershipStatus('Grace') . ') AND end_date >= "' . $from->format('Y-m-d') . '" AND end_date <= "' . $to->format('Y-m-d') . '"';
				//				echo $query;
				return CRM_Core_DAO::singleValueQuery($query);
				break;
